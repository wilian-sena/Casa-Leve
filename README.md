...
**Netlify** (alternativa, funciona com o repositório privado)
Já existe `netlify.toml` com `command = "npm run build"` e `publish = "out"`.
Basta ligar o repositório ao projeto Netlify — ou, a partir desta pasta:
...
**GitHub Pages (é como está publicada)**

`.github/workflows/pages.yml` corre a cada push para `main`: lint, tipos, testes, build e publicação em

```
https://wilian-sena.github.io/Casa-Leve/
```

Se algum teste falhar, não publica. É preciso ativar uma vez em **Settings → Pages → Source: GitHub Actions**,
e o repositório tem de ser **público** (Pages em repositórios privados exige plano pago).

Para construir com o mesmo endereço localmente:

```bash
NEXT_PUBLIC_BASE_PATH=/Casa-Leve npm run build
```
